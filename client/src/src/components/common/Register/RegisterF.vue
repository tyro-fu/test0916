    ...
    <div class="login-form">
      ...
      <div class="padding-m-s-left-50">
        ...
        <div class="submit-box">
          <el-button class="login-btn" @click="registerCheck">注册</el-button>
          <span class="reg">
            have an account?to
...
import Lbottom from "../Login/Lbottom";
export default {
  name: "loginContent",
  components: { Lbottom },
  data() {
  ...
  methods: {
    ...
    registerCheck() {
      if (this.userName == "") {
        this.messAlert("用户名不能为空，请重新输入", "error");
      } else if (this.userName.length < 4) {
        this.messAlert("用户名不能长度不能小于4个字符，请重新输入", "error");
      } else if (this.password == "") {
        this.messAlert("密码不能为空，请重新输入", "error");
      } else if (this.password1 == "") {
        this.messAlert("密码不能为空，请重新输入", "error");
      } else if (this.password !== this.password1) {
        this.messAlert("两次密码输入不相等，请重新输入", "error");
      } else {
        net
          .post("/register", {
          ...
          .then(res => {
            if (res.data.code == 1) {
              ...
              this.messAlert("用户已存在，请重新注册", "error");
            }
